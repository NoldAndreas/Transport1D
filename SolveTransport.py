...
class Transport:
    ...
    def u_m(self,m):
        ...
        if((self.params['uptake_mRNA'] == 'const') or (eta_max_m == 0)):
            ...
            fac = m*np.tanh(m/eta_max_m*eta_0);
        else:
    ...
    def u_p(self,p):
        ...
        if((self.params['uptake_proteins'] == 'const') or (eta_max_p == 0)):
            ...
            fac = p*np.tanh(p/eta_max_p*eta_0);
        else:
    ...
    def __computeCostItems(self):
        ...
        J_p   = self.params['v_p']*self.res_p.sol(0)[0]-self.params['D_p']*self.res_p.sol(0)[1];
        J_m   = self.params['v_m']*self.res_m.sol(0)[0]-self.params['D_m']*self.res_m.sol(0)[1];
    ...
    def SolveTransportEqs(self,N):
        ...
        if(self.params['uptake_mRNA']=='const'):
    ...
    #        raise ValueError("boundary condition must fix influx or end concentration");
        ...
        if(self.params['uptake_proteins']=='const'):
